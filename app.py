...
def create_activity_heatmap(df_input):
    ...
    full_df = full_df.merge(df[['Data', 'Total']], on='Data', how='left')
    full_df['Total'] = full_df['Total'].fillna(0)
    
    # Adicionar informações de posicionamento no grid
    full_df['week'] = ((full_df['Data'] - start_date).dt.days // 7)
    full_df['day_of_week'] = full_df['Data'].dt.dayofweek
    
    # Criar categorias de intensidade
    max_sales = full_df['Total'].max()
    if max_sales > 0:
        full_df['intensity'] = pd.cut(
            full_df['Total'], 
            bins=[-0.1, 0, max_sales*0.25, max_sales*0.5, max_sales*0.75, max_sales],
            labels=['Sem vendas', 'Baixa', 'Média', 'Alta', 'Muito Alta']
        )
    else:
        full_df['intensity'] = 'Sem vendas'
    
    # Filtrar apenas dias do ano atual para exibição
    display_df = full_df[full_df['is_current_year']].copy()
    ...
    # Criar heatmap
    heatmap = alt.Chart(display_df).mark_rect(
        stroke='white',
        strokeWidth=1
    ).encode(
        x=alt.X(
        ...
        color=alt.Color(
            ...
            legend=alt.Legend(title="Vendas (R$)")
        ),
        tooltip=[
            alt.Tooltip('Data:T', title='Data', format='%d/%m/%Y'),
            alt.Tooltip('Total:Q', title='Vendas (R$)', format=',.2f'),
            alt.Tooltip('intensity:N', title='Intensidade')
        ]
    ).properties(
        title=alt.TitleParams(
            text=f"Heatmap de Atividade de Vendas - {current_year}",
            fontSize=18,
            anchor='start'
        ),
        width=800,
...
def main():
    ...
    <div class="logo-fire-container">
        <img src="data:image/svg+xml;base64," class="fire-logo" alt="Clips Burger Logo"/>
    ...
    # Título principal
    st.title("🍔 Dashboard de Vendas - Clips Burger")
    st.markdown("---")
    ...
    with st.sidebar:
        ...
        if not df_processed.empty and 'Ano' in df_processed.columns:
            ...
            if selected_anos:
                df_filtered_year = df_processed[df_processed['Ano'].isin(selected_anos)]
                
                if not df_filtered_year.empty and 'MêsNome' in df_filtered_year.columns:
                    meses_disponiveis = df_filtered_year['MêsNome'].dropna().unique()
                    meses_ordenados = [m for m in meses_ordem if m in meses_disponiveis]
                    ...
                    if selected_meses:
                        df_filtered = df_filtered_year[df_filtered_year['MêsNome'].isin(selected_meses)]
                    else:
                        df_filtered = df_filtered_year
                else:
                    df_filtered = df_filtered_year
                    selected_meses = []
            else:
    ...
    # Tabs principais
    tab1, tab2, tab3, tab4 = st.tabs(["📊 Dashboard", "📈 Análises", "💰 Financeiro", "➕ Adicionar Dados"])
    ...
    with tab1:
        if df_filtered.empty:
            ...
            with col3:
                dias_vendas = len(df_filtered[df_filtered['Total'] > 0])
                st.metric("📅 Dias com Vendas", dias_vendas)
            ...
            with col1:
                ...
                if radial_chart:
                    ...
                    st.info("Sem dados suficientes para o gráfico de métodos de pagamento.")
    ...
    with tab2:
        if df_filtered.empty:
            ...
            if weekday_chart:
                ...
                if best_weekday:
                    st.success(f"🏆 Melhor dia da semana: **{best_weekday}**")
            ...
            st.subheader("🔥 Heatmap de Atividade de Vendas")
            heatmap = create_activity_heatmap(df_filtered)
            if heatmap:
                st.altair_chart(heatmap, use_container_width=True)
                st.info("📝 **Nota:** O heatmap mostra todos os dados do ano, independente dos filtros de mês aplicados.")
    ...
    with tab4:
        ...
        with st.form("add_sale_form"):
            ...
            total_previsto = cartao + dinheiro + pix
            st.info(f"💰 Total da venda: R$ {total_previsto:.2f}")
