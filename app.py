...
def main():
    ...
    with tab3:
        ...
        if not df_raw.empty and 'Data' in df.columns:
            ...
            # Layout com colunas para as estatísticas
            col1, col2 = st.columns(2)
            
            # Coluna 1: Estatísticas Gerais
            with col1:
            ...
            # Coluna 2: Análise Temporal
            with col2:
                st.subheader("📅 Análise Temporal")
                
                # Se houver dados suficientes, mostra tendências
                if total_vendas > 1:
                    ...
                    if 'Data' in df_filtered.columns:
                        ...
                        if not vendas_por_dia.empty:
                            ...
                            # Criando gráfico de barras para média por dia da semana
            ...
            # Seção inferior com estatísticas adicionais
            st.markdown("---")
            ...
            with col_b:
                ...
                if total_vendas > 1:
                    ...
                    st.markdown(f"**📅 Projeção Mensal:** R$ {projecao_mensal:.2f}")
                    
                    # Meta diária para atingir 20% a mais que a projeção mensal
                    meta_mensal = projecao_mensal * 1.2
                    meta_diaria = meta_mensal / 30
                    st.markdown(f"**🌟 Meta Diária:** R$ {meta_diaria:.2f}")
                    ...
                    if 'AnoMês' in df_filtered.columns and df_filtered['AnoMês'].nunique() > 1:
                        ...
                        if len(vendas_mensais) >= 3:  # Pelo menos 3 meses para calcular taxa
                            ...
                            if taxas:
                                ...
                                st.markdown(f"**📈 Taxa Média de Crescimento:** {taxa_media*100:.1f}%")
                                
                                # Previsão para próximo mês com base na taxa média
                                ultimo_mes = vendas_mensais.iloc[-1]['Total']
                                previsao_proximo = ultimo_mes * (1 + taxa_media)
                                st.markdown(f"**🔮 Previsão Próximo Mês:** R$ {previsao_proximo:.2f}")
            
            with col_c:
                # Estatísticas de frequência
                if 'Data' in df_filtered.columns:
                    st.markdown("### 🗓️ Frequência")
                    
                    # Dias sem vendas no período selecionado
                    if not df_filtered.empty:
                        ...
                        st.markdown(f"**⚠️ Dias Sem Vendas:** {dias_sem_vendas}")
                        ...
                        if len(datas_com_vendas) > 1:
                            ...
                            st.markdown(f"**⏱️ Intervalo Médio:** {dias_entre_vendas:.1f} dias")
                            
                            # Calcular dias da semana mais frequentes
                            if len(df_filtered) > 3:  # Mais de 3 registros
                                st.markdown(f"**🏆 Dia Mais Frequente:** {dia_mais_vendas}")
                                
            # Nova seção de comparativos e tendências
            st.markdown("---")
            st.subheader("📊 Comparativos e Tendências")
                ...
                if 'Data' in df_filtered.columns and len(df_filtered) > 6:  # Pelo menos uma semana
                    ...
                    if 'DiaSemana' in df_filtered.columns:
                        ...
                        if total_semana > 0:
                            ...
                            st.markdown(f"**🔝 Melhor dia:** {melhor_dia['DiaSemana']} ({melhor_dia['Porcentagem']:.1f}% do total)")
                            st.markdown(f"**🔻 Pior dia:** {pior_dia['DiaSemana']} ({pior_dia['Porcentagem']:.1f}% do total)")
            
            with comp_col2:
                # Análise de proporção de métodos de pagamento ao longo do tempo
                if 'AnoMês' in df_filtered.columns and df_filtered['AnoMês'].nunique() >= 2:
                    st.markdown("### 💰 Evolução dos Métodos de Pagamento")
                    ...
                    if df_pagamentos.shape[0] >= 3:  # Pelo menos 3 meses
                        ...
                        if total_primeiro > 0 and total_ultimo > 0:
                            ...
                            st.markdown(f"💳 **Cartão:** {delta_cartao:+.1f}%")
                            st.markdown(f"💵 **Dinheiro:** {delta_dinheiro:+.1f}%")
                            st.markdown(f"📱 **PIX:** {delta_pix:+.1f}%")
            ...
            if not df_filtered.empty and len(df_filtered) > 1:
                ...
                if 'AnoMês' in df_filtered.columns:
                    ...
                    if len(meses_ordenados) >= 2:
                        ...
                        with kpi_col3:
                            # Considerando PIX + Cartão como pagamentos digitais
            ...
            st.info("Não há dados disponíveis para exibir estatísticas. Registre vendas para ver as análises estatísticas.")
