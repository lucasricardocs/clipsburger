...
def create_area_chart_with_gradient(df):
    """Cria gráfico de área com gradiente substituindo o gráfico de montanha.""    if df.empty or \'Data\' not in df.columns or \'Total\' not in df.columns:\n        return None\n\n    df_sorted = df.sort_values(\'Data\').copy()\n    # Calcula a soma acumulada DENTRO dos dados filtrados (mês/ano selecionado)\n    df_sorted[\'Total_Acumulado\'] = df_sorted[\'Total\'].cumsum()  
    if df_sorted.empty:
        return None
    ...
    area_chart = alt.Chart(df_sorted).mark_area(
        ...
        title=alt.TitleParams(            text=\"Evolução Acumulada das Vendas no Período\",
            fontSize=18,
            anchor='start'
        ),
        height=500,
...
if __name__ == "__main__":
    ...
    .footer {
        ...
        bottom: 0;
        width: 100%;
        background: linear-gradient(90deg, #e0f2ff, #ffffff);
    ...
    .footer a {
        color: #0056b3; /* Azul mais vibrante para links */
    ...
    <div class="footer">
        <div class="footer-content">
            <span>🍔 Clip's Burger &copy; {datetime.now().year}</span>
            <span>|</span>
            <span>Desenvolvido com ❤️ por <a href="#" target="_blank">Seu Nome/Empresa</a></span>
            <span>|</span>
            <span>Análises que impulsionam o sabor! ✨</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
