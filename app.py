...
def create_payment_methods_chart(df):
    ...
    # Adicionar texto de porcentagem - Corrigido para usar apenas o formato nativo do Altair
    text = alt.Chart(metodo_pagamento).mark_text(radius=150, size=16).encode(
        theta=alt.Theta(field="Valor", type="quantitative"),
        text=alt.Text('Porcentagem:Q', format='.0f%'),  # Usando apenas o formato nativo do Altair
